...
def display_data_by_year_exp_sub_cat_ak(year):
    ...
    # Filter data by year
    year_data = []
    total_sub_expense_year = 0  # Initialize total expense for the year
    
    for item in data:
        for key, value in item.items():
            if key.startswith("Sub Expense") and key.endswith(f"{year}(millions)"):
                total_sub_expense_year += value  # Add expense to total for the year
    ...
    for item in data:
        for key, value in item.items():
            if key.startswith("Sub Expense") and key.endswith(f"{year}(millions)"):
                ...
                expense = value
                percentage_share = (expense / total_sub_expense_year) * 100 if total_sub_expense_year != 0 else 0
                year_data.append({"Sub-Category Name": category_name, "Expense": expense, "Share": percentage_share})
...
def display_data_by_year_rev_sub_cat_ak(year):
    ...
    # Filter data by year
    year_data = []
    total_sub_revenue_year = 0  # Initialize total expense for the year
    
    for item in data:
        for key, value in item.items():
            if key.startswith("Sub Expense") and key.endswith(f"{year}(millions)"):
                total_sub_revenue_year += value  # Add expense to total for the year
    ...
    for item in data:
        for key, value in item.items():
            if key.startswith("Sub Expense") and key.endswith(f"{year}(millions)"):
                category_name = item["Sub-Category Name"]
                expense = value
                percentage_share = (expense / total_sub_revenue_year) * 100 if total_sub_revenue_year != 0 else 0
                year_data.append({"Sub-Category Name": category_name, "Expense": expense, "Share": percentage_share})
